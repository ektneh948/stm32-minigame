...
<img width="900" alt="feature1" src="{FEATURE_IMAGE_1}" />
...
<img width="900" alt="feature2" src="{FEATURE_IMAGE_2}" />
...
* 프로젝트 인원: **2명**
* 본인 역할: **임베디드 펌웨어(입력/출력/타이머/게임 로직 통합)**
...
* 원인: 매 프레임 `lcd_clear()`를 수행하면 전체 화면이 반복 초기화되어 점멸 체감이 커짐
* 해결 방법: Clear 대신 Overwrite 방식으로 필요한 위치만 갱신

* `LCD_Display_Charactor()`에서 방문 칸은 공백(0x20), 미방문 칸은 먹이(0xA5)로 덮어쓰기
* 결과: 화면 점멸이 크게 줄고, 움직임이 더 자연스럽게 보임

---

### 문제 2) **문어(Enemy)가 너무 빠르거나 불규칙하게 이동**

* 원인: 루프 딜레이/출력 주기와 무관하게 이동하면 체감 난이도가 급격히 변함
* 해결 방법: TIM2 인터럽트 기반 `clk_pulse` 토글로 이동 타이밍을 고정

* `clock_before`로 토글 변화 시점에만 1회 이동하도록 제한
* 레벨업 시 `TIM2->PSC` 조절로 속도만 단계적으로 증가
* 결과: 난이도 곡선(LEVEL1→3)이 명확해짐
